...
/*   Updated: 2023/06/01 11:40:53 by josorteg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_win(t_painter *painter)
{
	...
	(0 || (table = malloc(painter->pict.iter_num * sizeof(int32_t))) \
	|| (i = 0) || (tmp.y = 0));
	if (!table)
		exit(1);
	while (++i < painter->pict.iter_num)
		table[i] = (i * 255 * 255 * 255 / painter->pict.iter_num);
